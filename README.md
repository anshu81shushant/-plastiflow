...
- **PDF downloads**: each order's Edit page has a "Download job sheet" button — a printable PDF with order details and its full production log, good for the shop floor. The All Orders page has "Download report" — a landscape PDF table of every currently-filtered order, useful for sharing a snapshot with someone who doesn't have app access.
- **Dashboard alerts panel**: overdue orders, low-stock materials, and machines currently marked "Down" now surface together in one place at the top of the dashboard, so you don't have to check three separate pages to know what needs attention today. Shows "All clear" when nothing needs action.
- **Production trend chart**: a simple 7-day bar chart on the dashboard showing units produced per day across all orders, pulling from your existing production log data — fills in automatically as you log daily production.
- **Real reorder forecasting**: the Materials page now shows two numbers per material — how much is committed to *active* orders (as before), and a new "actual usage rate" based on your real production history over the last 30 days, even for materials with zero active orders right now. This catches the case the old logic missed entirely: steady background consumption with nothing currently "in progress." Needs at least a few days of logged production against orders using that material to show anything — verified with test data (30kg used over 10 real days correctly computes to 3kg/day, 16 days until empty on 50kg stock).
