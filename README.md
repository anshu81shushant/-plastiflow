...
- **Scan to fill (demo)**: visit `/scan-demo` on your live site to try scanning a Purchase Order or supplier bill photo — it reads the document and extracts customer name, item, quantity, due date, and more (or material name, quantity, price for bills). A sample test PO image (`sample_po_for_testing.png`) is included in this zip so you can try it immediately without needing a real document. This is a demo page only right now — it shows what got extracted but doesn't save anywhere yet. Needs `ANTHROPIC_API_KEY` set (same key used for other AI features). **Important honesty note**: extraction accuracy depends entirely on document clarity — messy handwriting, low-light photos, or unusual formats will extract less reliably. Always treat it as a starting point to review, not a guaranteed-correct auto-fill, especially for numbers like price and quantity.
