...
// Historical-usage forecast: looks at actual material consumption from production logs
// over the recent past (not just orders currently active), giving a real burn rate even
// when nothing is actively in progress. Falls back gracefully with less data.
export function materialHistoricalForecast(material, relevantProductionLogs, lookbackDays = 30) {
  const cutoff = new Date();
  cutoff.setDate(cutoff.getDate() - lookbackDays);

  const recentLogs = relevantProductionLogs.filter((log) => new Date(log.log_date) >= cutoff);

  if (recentLogs.length === 0) {
    return { hasHistory: false, dailyBurnKg: 0, daysUntilEmpty: null, totalConsumedKg: 0, lookbackDays };
  }

  const totalConsumedKg = recentLogs.reduce((sum, log) => sum + (log.consumedKg || 0), 0);

  // Use the actual span of days with real activity, not the full lookback window,
  // so a burst of production 3 days ago doesn't get diluted into a 30-day average.
  const dates = recentLogs.map((l) => new Date(l.log_date).getTime());
  const earliestLog = new Date(Math.min(...dates));
  const daysSpan = Math.max(1, Math.ceil((new Date() - earliestLog) / 86400000));

  const dailyBurnKg = totalConsumedKg / daysSpan;
  const daysUntilEmpty = dailyBurnKg > 0 ? Math.floor(material.stock_kg / dailyBurnKg) : null;

  return { hasHistory: true, dailyBurnKg, daysUntilEmpty, totalConsumedKg, daysSpan, lookbackDays };
}
